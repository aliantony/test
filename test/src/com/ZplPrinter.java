...
public class ZplPrinter {  
    ...
    public static void main(String[] args) {  
        ZplPrinter p = new ZplPrinter("//127.0.0.1/TSC TTP-244 Pro");  
        //1.打印单个条码  
        ...
        for (int off = 0; off < ch.length;) {  
            if ((ch[off] & 0x00ff) >= 0xA0) {  
                ...
                for (int j = 0; j < 72; j += 3) {  
                    qcode = dotFont[j + offset] & 0x00ff;  
                    wcode = dotFont[j + offset + 1] & 0x00ff;  
                    int qcode1 = dotFont[j + offset + 2] & 0x00ff;  
                    begin += String.format("%02X%02X%02X\n", qcode, wcode, qcode1);  
                }  
                x = x + ms * mx;  
                off = off + 2;  
            } else if ((ch[off] & 0x00FF) < 0xA0) {  
